...
const tetris = async (...args) => {
  try {
    ...
    const drawBoard = () => {
      ...
      for (let y = 0; y < HEIGHT; y++) {
        ...
        for (let x = 0; x < WIDTH; x++) {
          ...
          if (
            currentPiece &&
            currentPiece.blocks.some(
              ([by, bx]) => by + currentPos.y === y && bx + currentPos.x === x
            )
          ) {
    ...
    const cleanup = () => {
      ...
      const highScore = config.getConfig()?.tetrisHighScore ?? -1;
      ...
      if (highScore < score) {
        ...
        config.addData({ tetrisHighScore: score });
